...
//
// IMPORTANT — avatars (here and in AUTHORS above) and POSTS' own `image`
// field all get fed into functions/og.png.js's generated share-card image.
// That renderer (Satori) can only decode PNG/JPEG — a .webp path there
// doesn't just look wrong, it makes the ENTIRE card generation throw,
// silently falling back to a flat generic icon for that item's shared
// link. Use .png/.jpg for these three fields specifically. (.webp is
// fine anywhere else on the site — nav icons, social row, etc. — since
// those never go through og.png.js.)
const POSTS = [
  /* {
    id: 'post-demo-1',
    ...
    avatar: 'assets/anon.png', // manually picked for this post — overrides author-demo-1's usual avatar
